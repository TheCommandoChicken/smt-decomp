...
struct object * GameInit(void) {
	...
	GameDataInit();
	PersistentObjects.intro_manager = CreateObject(
		IntroManagerProc, EmptyProc, FirstObjectPtrPtr->first, 
		1, OBJ_TYPE_DEFAULT, sizeof(struct intro_manager)
	);
	intro = PersistentObjects.intro_manager->data;
	bzero((u8 *)intro, sizeof(*intro));
	intro->data_struct = func_80019C58(0, (void *)D_80010308, 7, 0, 0, 1);
...
void IntroManagerProc(struct object * self) {
	struct intro_manager * dat = self->data;
...
void func_800144D4(s8 arg0) {
	struct intro_manager * dat = PersistentObjects.intro_manager->data;
	dat->frame_counter = 1;
	dat->unk8 = arg0;
...
void func_800145CC(s32 (*arg0)(void)) {
	...
	func_80018F6C(0, 0);
	obj = CreateObject(
		func_80014698, NULL, FirstObjectPtrPtr->first, 
...
void func_80014698(struct object * self) {
	struct unk_data_0 * dat = self->data;
...
void func_80014928(s32 arg0) {
	PersistentObjects.unk0->unk88 = 
	PersistentObjects.unk0->unk8A = 
	PersistentObjects.unk0->unk10[arg0];
...
struct object * func_80014A2C(void) {
	struct object * obj = CreateObject(
		func_80014AF4, NULL, FirstObjectPtrPtr->first,
		0xC00000, OBJ_TYPE_DEFAULT, sizeof(struct unk_data_1)
	);
	if (obj == NULL) return NULL;
	bzero(obj->data, sizeof(*obj));
...
void func_80014AF4(struct object * self) {
	...
	struct unk_data_1 * dat = self->data;
...
void func_80014CD4(struct unk_data_1 * dat, s32 arg1) {
	DVECTOR pos_lookup[2] = {
		{0x0018, 0x0088},
		{0x0018, 0x6F}
	};
	s32 temp_s0 = func_80044114() == 0;
	u16 * temp = func_8004B888(arg1 + 772);

	func_80021DBC(temp, pos_lookup[temp_s0].vx + 4, pos_lookup[temp_s0].vy + 2, 5, 0, 0, 21);

	if (func_80044114() == 0) {
		func_80020AAC(pos_lookup[temp_s0].vx, pos_lookup[temp_s0].vy, 272, 16, 21, &dat->f[dat->buffer], 0);
	} else {
		func_800209F0(pos_lookup[temp_s0].vx, pos_lookup[temp_s0].vy, 272, 16, 21, &dat->g[dat->buffer], 1);
	}
...
/* 100% */
void func_800150A0(struct object * self) {
	struct unk_data_1 * dat = self->data;

	switch (dat->counter) {
	case 0:
		dat->unk0 = func_800494D0(0xC00000);
		dat->counter += 1;
	case 1:
		if (dat->unk0->initialized != 1) {
			self->proc_func = func_80014AF4;
			RemoveObject(dat->unk0);
			dat->unk0 = NULL;
			dat->counter = 0;
...
/* 100% */
struct object * func_8001513C(struct unk_data_1 * self) {
	s32 i;

	for (i = 0; i < 3; i++) {
		self->unk4[i] = func_8004B888(i + 0x297);
	}
	return func_80065F38(4, 127, 41, 66, 3, 0, &self->unk4, 3, 5, 0xC00000, 0, 0, 1);
}
